    ...
    for i in range(num_states):
        ...
        if s1 == "terminal":
            ...
            if t1 == constants.DP.MAX_TIMESTEPS - 1:
                T[i][state_to_id["terminal"]][0] = 1
                T[i][state_to_id["terminal"]][1] = 1
            else:
...
def one_step_lookahead(state, V, num_actions, num_states, T, discount_factor, R, next_state_lookahead):
    ...
    for i in range(n_states):
        ...
        for j in range(n_states):
            for k in range(n_actions):
                if T[i][j][k] > 0.0 and j not in next_states:
